...
/***/ (function(module, exports, __webpack_require__) {
	...
	function Feedback(props){(0,_classCallCheck3.default)(this,Feedback);var _this=(0,_possibleConstructorReturn3.default)(this,(Feedback.__proto__||(0,_getPrototypeOf2.default)(Feedback)).call(this,
	props));
	_this.state={
	feedbackList:[],
	current:1,
	...
	['changePage','getContact'].forEach(function(m){return _this[m]=_this[m].bind(_this);});return _this;
	
	}(0,_createClass3.default)(Feedback,[{key:'componentWillMount',value:function componentWillMount()
	...
	data){
	if(data.status){
	return data.name+', '+data.company+', '+data.department+', '+data.position+', '+(data.mobile?'手机:'+data.mobile:'')+', '+(data.email?'邮箱:'+data.email:'')+', \u67E5\u770B\u4E86\u60A8\u7684\u9879\u76EE';
	}else{
	return data.name+', \u67E5\u770B\u4E86\u60A8\u7684\u9879\u76EE';
	}
	}},{key:'getProjectLink',value:function getProjectLink(
	
	data){
	return _react2.default.createElement(_reactRouter.Link,{to:'/project/detail/'+data.projectDetail.id+'?type=normal'},data.projectDetail.name);
	}},{key:'renderPage',value:function renderPage(
	
	page){
	var that=this;
	ajax({
	url:"/project/feedback/getInfoList",
	data:{page:page},
	callback:function callback(data){
	that.setState({
	feedbackList:data.infoList,
	total:data.total});
	
	}});
	
	}},{key:'getContact',value:function getContact(
	
	id){
	var that=this;
	ajax({
	url:"/project/feedback/readInfo",
	data:{id:id},
	callback:function callback(data){
	_antd.message.success("查看成功！",1,function(){
	that.state.feedbackList.filter(function(item){return item.id==id;})[0].status=1;
	that.setState({feedbackList:that.state.feedbackList});
	that.props.changeFeedbackNum(--that.props.basicInfo.feedback);
	});
	}});
	...
	_react2.default.createElement(_antd.Row,{className:'menuTitle'},'\u6211\u6536\u5230\u7684\u53CD\u9988(\u540C\u65F6\u53D1\u90AE\u4EF6\u63D0\u9192)'),
	_react2.default.createElement(_antd.Row,{className:'feedbackList'},
	this.state.feedbackList.map(function(item,index){return(
	_react2.default.createElement(_antd.Row,{className:index%2==0?'each gray':'each',key:item.id},
	_react2.default.createElement(_antd.Row,{className:'date'},item.date),
	_react2.default.createElement(_antd.Row,null,
	_this2.getContent(item),_this2.getProjectLink(item),',\u8BF7\u5C3D\u5FEB\u4E0E\u4ED6\u8054\u7CFB\u3002',
	item.status?null:
	_react2.default.createElement('a',{href:'javascript:;',onClick:_this2.getContact.bind(_this2,item.id)},'\u67E5\u770B\u4ED6\u7684\u8054\u7CFB\u65B9\u5F0F'))));})),
...
//# sourceMappingURL=projectSpaceFeedback.dc5f1356aed4d7dafada.chunk.js.map
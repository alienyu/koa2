...
/***/ (function(module, exports, __webpack_require__) {
	...
	function Info(props){(0,_classCallCheck3.default)(this,Info);var _this=(0,_possibleConstructorReturn3.default)(this,(Info.__proto__||(0,_getPrototypeOf2.default)(Info)).call(this,
	props));_this.
	...
	handleCancel=function(){return _this.setState({previewVisible:false});};_this.state={featureList:[],uploaderList:{bizLicenceFileList:[],companyFileList:[],personalFileList:[],certificationFileList:[]},feature:1,previewVisible:false,previewImage:''};['changeFeature','handleSubmit','handlePreview','handleCancel','handleBizLicence','handleCompany','handlePersonal','handleCertification'].forEach(function(m){return _this[m]=_this[m].bind(_this);});return _this;}(0,_createClass3.default)(Info,[{key:'handlePreview',value:function handlePreview(file){this.setState({previewImage:file.url||file.thumbUrl,previewVisible:true});}},{key:'handleBizLicence',value:function handleBizLicence(info){var that=this;var file=info.file;var fileList=info.fileList;if(fileList.length>0){if(fileList.length>1){fileList=[file];}if(fileList[0].status=="error"){fileList=[];_antd.message.error("上传图片失败!");}if(fileList[0].status=="done"){fileList[0].url=''+fileList[0].response.url;}}this.state.uploaderList.bizLicenceFileList=fileList;this.setState({uploaderList:this.state.uploaderList});}},{key:'handleCompany',value:function handleCompany(info){var that=this;var file=info.file;var fileList=info.fileList;if(fileList.length>0){if(fileList.length>1){fileList=[file];}if(fileList[0].status=="error"){fileList=[];_antd.message.error("上传图片失败!");}if(fileList[0].status=="done"){fileList[0].url=''+fileList[0].response.url;}}this.state.uploaderList.companyFileList=fileList;this.setState({uploaderList:this.state.uploaderList});}},{key:'handlePersonal',value:function handlePersonal(info){var that=this;var file=info.file;var fileList=info.fileList;if(fileList.length>0){if(fileList.length>1){fileList=[file];}if(fileList[0].status=="error"){fileList=[];_antd.message.error("上传图片失败!");}if(fileList[0].status=="done"){fileList[0].url=''+fileList[0].response.url;}}this.state.uploaderList.personalFileList=fileList;this.setState({uploaderList:this.state.uploaderList});}},{key:'handleCertification',value:function handleCertification(info){var that=this;var file=info.file;var fileList=info.fileList;if(fileList.length>0){if(fileList.length>1){fileList=[file];}if(fileList[0].status=="error"){fileList=[];_antd.message.error("上传图片失败!");}if(fileList[0].status=="done"){fileList[0].url=''+fileList[0].response.url;}}this.state.uploaderList.certificationFileList=fileList;this.setState({uploaderList:this.state.uploaderList});}},{key:'handleSubmit',value:function handleSubmit()
	
	{
	var that=this;
	if(that.state.uploaderList.bizLicenceFileList.length==0||!this.state.uploaderList.bizLicenceFileList[0].url){
	_antd.message.error("请上传营业执照正本！");
	return false;
	}
	if(that.state.uploaderList.companyFileList.length==0||!this.state.uploaderList.companyFileList[0].url){
	_antd.message.error("请上传公司名片！");
	return false;
	}
	if(that.state.uploaderList.personalFileList.length==0||!this.state.uploaderList.personalFileList[0].url){
	_antd.message.error("请上传个人照片！");
	return false;
	}
	if(that.state.uploaderList.certificationFileList.length==0||!this.state.uploaderList.certificationFileList[0].url){
	_antd.message.error("请上传身份证正面！");
	return false;
	}
	ajax({
	url:"/info/setAuth",
	...
	bizLicence:this.state.uploaderList.bizLicenceFileList[0].url,
	company:this.state.uploaderList.companyFileList[0].url,
	personal:this.state.uploaderList.personalFileList[0].url,
	certification:this.state.uploaderList.certificationFileList[0].url},
	
	callback:function callback(data){
	_antd.message.success("提交成功！",1,function(){
	that.props.changeAuthStatus(1);
	});
	}});
	
	
	}},{key:'changeFeature',value:function changeFeature(
	feature){
	...
	var that=this;
	ajax({
	url:"/info/getAuthInfo",
	data:{type:'fund'},
	callback:function callback(data){
	var bizLicence,company,personal,certification=[];
	var imgList=['bizLicence','company','personal','certification'];
	...
	uploaderList:{
	bizLicenceFileList:bizLicence,
	companyFileList:company,
	personalFileList:personal,
	certificationFileList:certification}});
	
	
	}});
	
	}},{key:'getUploadText',value:function getUploadText(
	
	key){
	switch(key){
	case"bizLicence":
	return"营业执照正本或者副本（无需盖章）";
	case"company":
	return"公司名片";
	case"personal":
	return"个人照片";
	...
	this.state,previewVisible=_state.previewVisible,previewImage=_state.previewImage;
	var uploadButton=
	_react2.default.createElement('div',null,
	_react2.default.createElement(_antd.Icon,{type:'plus'}),
	_react2.default.createElement('div',{className:'ant-upload-text'},'Upload'));
	
	
	
	return(
	_react2.default.createElement(_antd.Row,{className:'authFrame'},
	_react2.default.createElement(_antd.Row,{className:'menuTitle'},'\u8BA4\u8BC1\u8D44\u6599\u4E0A\u4F20'),
	_react2.default.createElement(_antd.Row,{className:'feature'},
	_react2.default.createElement(_antd.Col,{span:2,offset:1,className:'label'},'\u8D44\u91D1\u6027\u8D28:'),
	_react2.default.createElement(_antd.Col,{span:8,offset:3},
	_react2.default.createElement(_antd.Select,{style:{width:'80%'},onChange:this.changeFeature,value:this.state.feature.toString()},
	this.state.featureList.map(function(item){return(
	_react2.default.createElement(Option,{value:item.key.toString(),key:item.key.toString()},item.value));})))),
	
	
	
	
	_react2.default.createElement(_antd.Row,{className:'upload'},
	_react2.default.createElement(_antd.Col,{span:2,offset:1,className:'label'},'\u4E0A\u4F20\u7167\u7247:'),
	_react2.default.createElement(_antd.Col,{span:20},
	['bizLicence','company','personal','certification'].map(function(item){return(
	_react2.default.createElement(_antd.Col,{span:12,key:item,style:{marginBottom:40}},
	_react2.default.createElement(_antd.Row,{type:'flex',justify:'center'},
	_react2.default.createElement(_antd.Upload,{
	action:'/v1/upload',
	listType:'picture-card',
	fileList:_this2.state.uploaderList[item+'FileList'],
	onPreview:_this2.handlePreview,
	onChange:_this2['handle'+item.substring(0,1).toUpperCase()+item.substring(1)]},
	
	_this2.state.uploaderList[item+'FileList'].length>=1?null:uploadButton)),
	
	
	_react2.default.createElement(_antd.Row,{style:{textAlign:'center'}},_this2.getUploadText(item))));}))),
	
	
	
	
	_react2.default.createElement(_antd.Row,null,
	_react2.default.createElement(_antd.Col,{span:6,offset:10},
	_react2.default.createElement(_antd.Button,{disabled:this.props.basicInfo.authStatus==0?false:true,type:'primary',onClick:this.handleSubmit,style:{width:'100%'}},'\u63D0\u4EA4\u8BA4\u8BC1'))),
	
	
	_react2.default.createElement(_antd.Row,null,
	_react2.default.createElement(_antd.Col,{span:10,offset:8,className:'prompt'},'*\u672C\u7AD9\u8BA4\u8BC1\u7684\u9879\u76EE\u65B9\u5FC5\u987B\u662F\u76F4\u63A5\u7684\u878D\u8D44\u4F01\u4E1A\uFF0C\u8D44\u91D1\u65B9\u5FC5\u987B\u662F\u76F4\u63A5\u7684\u8D44\u91D1\u65B9\u6216\u8005\u91D1\u878D\u673A\u6784\uFF0C\u8C22\u7EDD\u4E2D\u4ECB\u3002')),
...
//# sourceMappingURL=fundSpaceAuth.db3eb0ed050ba238ebfc.chunk.js.map